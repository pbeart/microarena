...
from .spike_vm import struct_mmio

from .mmio_tcp_server import MMIOTcpServer
...
class SpikeSimDriver[MMIO_STRUCT_R_T: ctypes.Structure, MMIO_STRUCT_RW_T: ctypes.Structure](simdriver.SimDriver):
    ...
    def start(self):
        ...
        self._spawn = pexpect.spawn(self._spike, args, encoding="utf-8", timeout=5)
    ...
    def get_mmio_struct_r(self) -> MMIO_STRUCT_R_T:
        return self.tcp_mmio_client.struct_r
    
    # the struct is rw so we don't have to worry about compiler-generated assembly
    # trying to do byte writes by reading, masking, writing back words (and so, if 
    # reads didn't work, the written word other than the byte being garbage)
    # HOWEVER
    # we also must be able to write to the rw struct from here because we want to 
    # have things like flags which should be reset as soon as they're set
    def set_mmio_struct_rw(self, structure: MMIO_STRUCT_RW_T):
        self.tcp_mmio_client.struct_rw = structure
